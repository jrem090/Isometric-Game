    ...
    public void drawMenus(Graphics g, Graphics2D graphics, Level toUse)
    {
        ...
        else //action menu
        {
            ...
            if(action.state == 2)
            {
                ...
                g.setFont(new Font("TimesRoman", Font.PLAIN, fontSize));
                graphics.setPaint(Color.white);
                g.drawString("1. " + active.activeJob.name,5,(0+1)*fontSize);
                g.drawString("2. " + active.passiveJob.name,5,(1+1)*fontSize);
                g.drawString("3. Inventory",5,(2+1)*fontSize);
            }
            else if(action.state!=23)
            {
                
                if(action.state == 21){}
                {
                    activated = active.activeJob;
                }
                if(action.state == 22)
                {
                    activated = active.passiveJob;
                }
                int fontSize = 20;
                if(activated.moveList.length<3)fontSize= 200/3;
                else fontSize = 200/(activated.moveList.length);
                //for(int i = 0; i < active.moveList.length; i++)
                for(int i = 0; i < activated.moveList.length; i++)
                {

                    g.setFont(new Font("TimesRoman", Font.PLAIN, fontSize));
                    graphics.setPaint(Color.white);
                    g.drawString((i+1)+". " + activated.moveList[i].name,5,(i+1)*fontSize);
                }
